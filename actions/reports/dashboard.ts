  ...
  return runWithOrganizationContext(organizationId, async () => {
    ...
    const [
      ...
      prismadb.crm_Opportunities.findMany({
        where: { organizationId, created_on: { gte: filters.dateFrom, lte: filters.dateTo }, deletedAt: null, status: "CLOSED" },
        select: { budget: true, currency: true },
      }),
      prismadb.crm_Opportunities.findMany({
        where: { organizationId, created_on: { gte: prev.dateFrom, lte: prev.dateTo }, deletedAt: null, status: "CLOSED" },
        select: { budget: true, currency: true },
      }),
      // pipelineValue
      prismadb.crm_Opportunities.findMany({
        where: { organizationId, created_on: { gte: filters.dateFrom, lte: filters.dateTo }, deletedAt: null, status: "ACTIVE" },
        select: { budget: true, currency: true },
      }),
      prismadb.crm_Opportunities.findMany({
        where: { organizationId, created_on: { gte: prev.dateFrom, lte: prev.dateTo }, deletedAt: null, status: "ACTIVE" },
        select: { budget: true, currency: true },
      }),
      // newLeads
      ...
      prismadb.crm_Leads.count({
        where: { organizationId, createdAt: { gte: prev.dateFrom, lte: prev.dateTo }, deletedAt: null },
      }),
      ...
      prismadb.crm_Contacts.count({
        where: { organizationId, created_on: { gte: prev.dateFrom, lte: prev.dateTo } },
      }),
      ...
      prismadb.tasks.count({
        where: { organizationId, createdAt: { gte: prev.dateFrom, lte: prev.dateTo } },
      }),
      // open tasks (ACTIVE = not completed)
      prismadb.tasks.count({
        where: { organizationId, createdAt: { gte: filters.dateFrom, lte: filters.dateTo }, taskStatus: "ACTIVE" },
      }),
      prismadb.tasks.count({
        where: { organizationId, createdAt: { gte: prev.dateFrom, lte: prev.dateTo }, taskStatus: "ACTIVE" },
      }),
      // campaignsSent
      prismadb.crm_campaign_sends.count({
        where: { organizationId, sent_at: { gte: filters.dateFrom, lte: filters.dateTo } },
      }),
      prismadb.crm_campaign_sends.count({
        where: { organizationId, sent_at: { gte: prev.dateFrom, lte: prev.dateTo } },
      }),
      // newAccounts
      ...
      prismadb.crm_Contracts.count({
        where: { organizationId, endDate: { gte: filters.dateFrom, lte: filters.dateTo }, deletedAt: null },
      }),
      prismadb.crm_Contracts.count({
        where: { organizationId, endDate: { gte: prev.dateFrom, lte: prev.dateTo }, deletedAt: null },
      }),
    ]);
