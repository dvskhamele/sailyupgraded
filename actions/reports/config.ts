...
export async function duplicateConfig(configId: string, newName: string) {
  ...
  return prismadb.crm_Report_Config.create({ data: { organizationId, name: newName, category: original[0].category, filters: original[0].filters as Prisma.InputJsonValue, isShared: false, createdBy: userId } });
}
