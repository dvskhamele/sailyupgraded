  ...
  async ({ event, step }) => {
    ...
    const campaign = await step.run("check-campaign-status", async () => {
      return prismadb.crm_campaigns.findUnique({
        where: { id: campaignId },
        select: { status: true, target_lists: true, organizationId: true },
      });
    });
    ...
    const step0 = await step.run("get-step-0", async () => {
      return runWithOrganizationContext(organizationId, () =>
        prismadb.crm_campaign_steps.findFirst({
    ...
    const followUpSteps = await step.run("get-follow-up-steps", async () => {
      return runWithOrganizationContext(organizationId, () =>
        prismadb.crm_campaign_steps.findMany({
